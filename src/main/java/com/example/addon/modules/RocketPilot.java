...
public class RocketPilot extends Module {
    ...
    public void onActivate() {
        ...
        if (origin != null && mc.player.getEntityPos().distanceTo(origin) > 100) {
            resetPatternState();
        }

        // Reset drunk spiral state if player has moved too far from the origin since last time
        if (drunkSpiralOrigin != null && mc.player.getEntityPos().distanceTo(drunkSpiralOrigin) > 100) {
            resetDrunkSpiralState();
        }
    ...
    private void ebouncePath(IBaritone baritone) {
        ebouncePath(baritone, mc.player.getEntityPos());
    }
    ...
    private void handleDrunkSpiralMode() {
        if (mc.player == null) return;
        if (drunkSpiralOrigin == null) drunkSpiralOrigin = mc.player.getEntityPos();
